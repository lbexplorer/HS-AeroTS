...
Accordingly, the study evaluates three linked but non-interchangeable outputs: (i) runtime detection and fault-domain predictions against real-flight labels, (ii) hierarchical attribution evidence through conservation, masking, and semantic-map agreement, and (iii) module rankings against controlled source-mutation ground truth. The third output is evaluated using corrected controlled replay and a separately defined matched-reference residual extension. Onset-conditioned historical runs with inadequate input coverage are retained only as an execution audit, not capability estimates. The following subsection presents the overall framework implementing these definitions.
