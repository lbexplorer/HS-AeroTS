...
The stricter protocols reduced the fixed chronological performance and changed the relative behavior of the complete five-class models. Under the 14-window purged protocol, Stage 1 AUPRC was 0.5936, Stage 2 Macro-F1 was 0.7983, Cascade Macro-F1 was 0.5444, and Direct Five-Class Macro-F1 was 0.5962. The Direct Five-Class result was therefore higher than the Cascade result under this protocol, reversing their ordering on the fixed chronological split. Figure~\ref{fig:protocol_robustness} summarizes these protocol-level comparisons.
...
The controlled PX4 replay provided the only module-level ground truth, but its evaluation exposed a detector-gated capability boundary. In the onset-conditioned analysis, which supplied the known 30-s injection time and used matched baseline evidence, overall Top-1, Top-3, and Top-5 recalls were 0.250, 0.333, and 0.333, respectively, with MRR of 0.3227 and mean EXAM of 0.3318. The conditional ranks were uneven across mutations: EKF2 innovation bias reached Top-1 in two of three runs, whereas INAV local-z freeze and Land Detector state inversion did not enter Top-5 in any run. These results quantify conditional ranking behavior after onset information is supplied.
...
\begin{figure}[H]
    ...
    \caption{Primary protocol comparison for Stage 1 detection and complete or conditional diagnosis. The fixed chronological, purged, and leave-log-out protocols are evaluated with their respective split definitions; leave-log-out values show five-seed means with standard deviations where available. The purged protocol reverses the fixed-split ordering of Cascade and Direct Five-Class Macro-F1.}
