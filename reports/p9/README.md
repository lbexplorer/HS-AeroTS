...
> **配对 replay 继续优化**：新增独立正常重复，修复 EKF generic 模式和配对时钟后，配对残差扩展达到检测 10/12、门控 Top-1/3/5 4/12、10/12、10/12，MRR 0.5833，独立正常误报 1/12。该结果属于新增开发候选；仍有两条 EKF 漏报、仅三条源航次和一条效应检查未通过，尚未支持可靠性确认。见 [配对优化报告与后续验证设计](paired_residual/README.md)。

> **2026-09-06 专项复评**：下面保留历史结果。已查明旧 replay 的输入回放不完整及填充尾段问题；只将同一输入暂存到 WSL 本地、复用原二进制重跑 P9，并限制有效遥测时间覆盖后，检测为 10/12，门控 Top-1/3/5 为 2/12、5/12、5/12，MRR 0.3259。但正常 baseline 也报警 10/12，原注入效应检查为 11/12 通过，因此仍不支持可靠端到端诊断结论。完整根因、逐 mutation 结果、校准/映射消融与复现命令见 [专项报告](stage1_reassessment/README.md)。
