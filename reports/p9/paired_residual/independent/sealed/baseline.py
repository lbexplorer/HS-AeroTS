...
def _ulog_time_bounds(ulog: Any, preserve_header_start: bool = False) -> tuple[int, int]:
    ...
    # P9 paired comparisons must not silently switch origins when only the
    # final timestamp is invalid. Opt-in preserves historical P1/P2 artifacts.
    if preserve_header_start and 0 <= start < derived_stop and (derived_stop - start) / 1_000_000 <= 7200:
        derived_start = start
...
def align_log_arrays(ulog_path: Path, channels: list[str], entry: dict[str, Any], frequency_hz: float, preserve_header_start: bool = False, start_override_us: int | None = None) -> tuple[np.ndarray, np.ndarray, np.ndarray, int, int]:
    ulog = ULog(str(ulog_path), message_name_filter_list=list(TOPIC_FIELDS))
    start, stop = _ulog_time_bounds(ulog, preserve_header_start=preserve_header_start)
    if start_override_us is not None:
        if not 0 <= start_override_us < stop:
            raise ValueError('invalid explicit resampling origin')
        start = int(start_override_us)
