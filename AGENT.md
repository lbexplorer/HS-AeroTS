...
阶段状态：**P1–P11 已完成（2026-08-21）**。P1/P2 验收规模保持为 1,389 条日志、1,892,063 行、87 通道和 218,537 个窗口；P2 五种子 AUPRC 为 0.7522±0.0039，P3 四类诊断 Macro-F1 为 0.8980±0.0020。P8 已按多数固件提交建立 18 个 topic 到 54 个 PX4 模块的静态映射。P9 为 Controlled PX4 ULog Replay with Source-Level Mutations：3 条真实 ULog、4 个 PX4 源码 mutation、24 次 replay；onset-conditioned Top-1/3/5 为 0.250/0.333/0.333，固定 P2 阈值检测召回为 0。P10 purged 下 Cascade 低于 Direct Five-Class。P11 五种子 leave-log-out 下 Stage 1 AUPRC 为 0.6296±0.0023，Stage 2 Macro-F1 为 0.9041±0.0029，Cascade 与 Direct Five-Class Macro-F1 分别为 0.5962±0.0048 和 0.6092±0.0043。
...
3. **P9 结果维护**：保持 Ubuntu-20.04、固定 PX4 commit、无 Gazebo/ROS/QGroundControl/NuttX 的 Controlled PX4 ULog Replay with Source-Level Mutations 协议；区分 onset-conditioned 定位指标与检测门控端到端指标，不得用前者掩盖 Stage 1 对 12 个 fault run 零检出的限制。
...
- `topic→module` 静态依赖只表示证据传播和软件模块嫌疑度，不等同于根因；只有具有已知注入模块的隔离 controlled mutation replay run 可报告软件模块故障定位 Top-k/MRR/EXAM。
...
- 预计运行超过 2 分钟的命令（训练、完整测试、benchmark、数据处理）不要同步阻塞执行。
- 长任务必须以后台进程启动，并将 stdout/stderr 重定向到日志文件。
- 启动后记录 PID，并立即返回继续监控。
- 每 30-60 秒检查一次：
  1. 进程是否仍存在；
  2. 日志是否持续增长；
  3. 是否出现 traceback / error / failed；
  4. 是否已经生成预期输出文件。
- 如果连续 5 分钟日志无变化，检查进程状态并报告疑似卡死，不要无限等待。
- 长任务完成后再更新 Todo 状态。
- 不要因为 Shell 调用一直没有返回而无限等待。
